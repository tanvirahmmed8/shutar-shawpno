...
use App\Http\Controllers\Admin\Finance\FinanceAccountController;
use App\Http\Controllers\Admin\Finance\FinanceDashboardController;
use App\Http\Controllers\Admin\Finance\FinanceExpenseController;
use App\Http\Controllers\Admin\Finance\FinanceJournalController;
use App\Http\Controllers\Admin\Finance\FinanceReconciliationController;
use App\Http\Controllers\Admin\Finance\FinanceReportController;
use App\Http\Controllers\Admin\Finance\FinanceActivityController;
use App\Http\Controllers\Admin\Finance\FinanceTransferController;
use App\Http\Controllers\Admin\Finance\PaymentAccountMappingController;
use App\Http\Controllers\Admin\Purchase\VendorController as PurchaseVendorController;
...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['admin']], function () {
    ...
    Route::group(['prefix' => 'orders', 'as' => 'orders.', 'middleware' => ['module:order_management']], function () {
        Route::controller(OrderController::class)->group(function () {
            ...
            Route::post(Order::PARTIAL_PAYMENT_STORE[URI], 'storePartialPayment')->name('partial-payments.store');
    ...
    Route::group(['prefix' => 'purchase', 'as' => 'purchase.', 'middleware' => ['module:purchase_management']], function () {
        ...
        Route::post('orders/from-requisition/{requisition}', [PurchaseOrderController::class, 'storeFromRequisition'])->name('orders.from-requisition');
        ...
        Route::post('grns/{grn}/approve', [PurchaseGrnController::class, 'approve'])->name('grns.approve');
        Route::post('grns/{grn}/reject', [PurchaseGrnController::class, 'reject'])->name('grns.reject');
        Route::post('grns/{grn}/mark-returned', [PurchaseGrnController::class, 'markReturned'])->name('grns.mark-returned');
        Route::post('grns/{grn}/retry-inventory', [PurchaseGrnController::class, 'retryInventory'])->name('grns.retry-inventory');

        Route::post('grns/{grn}/returns', [ReturnToVendorController::class, 'store'])->name('grns.returns.store');
        Route::post('grn-returns/{return}/ship', [ReturnToVendorController::class, 'ship'])->name('grns.returns.ship');
        Route::post('grn-returns/{return}/close', [ReturnToVendorController::class, 'close'])->name('grns.returns.close');
    });

    Route::group(['prefix' => 'accounts-finance', 'as' => 'accounts-finance.', 'middleware' => ['module:accounts_finance']], function () {
        Route::get('/', [FinanceDashboardController::class, 'index'])->name('dashboard');

        Route::controller(FinanceAccountController::class)->group(function () {
            Route::get('accounts/export', 'export')->name('accounts.export');
            Route::post('accounts/import', 'import')->name('accounts.import');
        });
        Route::resource('accounts', FinanceAccountController::class)->except(['show']);

        Route::controller(PaymentAccountMappingController::class)->group(function () {
            Route::get('payment-mappings', 'edit')->name('payment-mappings.edit');
            Route::post('payment-mappings', 'update')->name('payment-mappings.update');
        });

        Route::resource('journals', FinanceJournalController::class);
        Route::post('journals/{journal}/post', [FinanceJournalController::class, 'post'])->name('journals.post');

        Route::resource('reconciliations', FinanceReconciliationController::class)->only(['index', 'create', 'store', 'show', 'destroy']);
        Route::post('reconciliations/{reconciliation}/rows/{row}/match', [FinanceReconciliationController::class, 'matchRow'])->name('reconciliations.rows.match');
        Route::patch('reconciliations/{reconciliation}/rows/{row}/unmatch', [FinanceReconciliationController::class, 'unmatchRow'])->name('reconciliations.rows.unmatch');

        Route::resource('expenses', FinanceExpenseController::class);
        Route::post('expenses/{expense}/approve', [FinanceExpenseController::class, 'approve'])->name('expenses.approve');

        Route::resource('transfers', FinanceTransferController::class)->only(['index', 'create', 'store', 'show', 'destroy']);
        Route::post('transfers/{transfer}/approve', [FinanceTransferController::class, 'approve'])->name('transfers.approve');

        Route::post('activities/preview', [FinanceActivityController::class, 'preview'])->name('activities.preview');
        Route::post('activities', [FinanceActivityController::class, 'store'])->name('activities.store');

        Route::controller(FinanceReportController::class)->prefix('reports')->as('reports.')->group(function () {
