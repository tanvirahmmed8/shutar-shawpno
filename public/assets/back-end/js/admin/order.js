...
$(document).ready(function () {
    ...
    initializePartialPaymentMatrix();
});
...
$(".payment-status-select").on('change', function () {
    ...
    let previousStatus = dropdown.data('current');
    let requestedStatus = dropdown.val();
...
function togglePartialPaymentAccountField(selectElement) {
    let wrapper = $('#partial-payment-account-custom-wrapper');
    if (!wrapper.length) {
        return;
    }
...
let partialPaymentMatrixData = null;

function parsePartialPaymentMatrixData() {
    if (partialPaymentMatrixData !== null) {
        return partialPaymentMatrixData;
    }

    let matrixElement = $('#partial-payment-account-matrix');
    if (!matrixElement.length) {
        return null;
    }

    try {
        partialPaymentMatrixData = JSON.parse(matrixElement.attr('data-matrix'));
    } catch (error) {
        partialPaymentMatrixData = null;
    }

    return partialPaymentMatrixData;
}

function getPartialPaymentOptions(method) {
    let matrix = parsePartialPaymentMatrixData();
    if (!matrix) {
        return [];
    }

    let optionsRoot = matrix.options || matrix;
    return optionsRoot[method] || optionsRoot['__default'] || [];
}

function getPartialPaymentDefault(method) {
    let matrix = parsePartialPaymentMatrixData();
    if (!matrix || !matrix.defaults) {
        return null;
    }

    return matrix.defaults[method] || matrix.defaults['__default'] || null;
}

function initializePartialPaymentMatrix() {
    let matrix = parsePartialPaymentMatrixData();
    let accountSelect = $('#partial-payment-account-select');
    let methodSelect = $('.partial-payment-method-select');

    if (!matrix || !accountSelect.length || !methodSelect.length) {
        togglePartialPaymentAccountField(accountSelect);
        return;
    }

    rebuildPartialPaymentAccounts(methodSelect.val(), false);

    methodSelect.on('change', function () {
        rebuildPartialPaymentAccounts($(this).val(), true);
    });
}

function rebuildPartialPaymentAccounts(method, triggerChange) {
    let accountSelect = $('#partial-payment-account-select');
    let matrix = parsePartialPaymentMatrixData();
    if (!accountSelect.length || !matrix) {
        return;
    }

    let options = getPartialPaymentOptions(method);
    let customLabel = matrix.custom_label || accountSelect.data('custom-label') || 'Custom account';
    let currentValue = accountSelect.val();
    let presetValue = accountSelect.data('selected');
    accountSelect.empty();

    options.forEach(function (option) {
        accountSelect.append(new Option(option.label, option.value));
    });
    accountSelect.append(new Option(customLabel, 'custom'));

    let allowedValues = options.map(function (option) { return option.value; });
    allowedValues.push('custom');

    let nextValue = currentValue;
    if (!allowedValues.includes(nextValue)) {
        if (presetValue && allowedValues.includes(presetValue)) {
            nextValue = presetValue;
        } else {
            let defaultValue = getPartialPaymentDefault(method);
            if (defaultValue && allowedValues.includes(defaultValue)) {
                nextValue = defaultValue;
            } else {
                nextValue = options.length ? options[0].value : 'custom';
            }
        }
    }

    accountSelect.data('selected', '');
    accountSelect.val(nextValue);
...
$(document).on('change', '.partial-payment-account-select', function () {
    togglePartialPaymentAccountField($(this));
});
