...
@section('content')
    <div class="content container-fluid ">
        ...
        @php($statusFilters = ['all','disburse','hold'])
        ...
        <div class="card mb-3 report-card">
            <div class="card-body">
                ...
                @php($statusQueryBase = request()->except('status','page'))
                <div class="status-filter mt-3">
                    @foreach($statusFilters as $state)
                        @php($query = array_merge($statusQueryBase, ['status'=>$state]))
                        @php($href = url()->current().'?'.http_build_query($query))
                        <a class="pill {{ $currentStatus === $state ? 'active' : '' }}" href="{{ $href }}">
                            {{ translate($state) }}
                        </a>
                    @endforeach
                </div>
