...
@section('content')
    <div class="content container-fluid ">
        ...
        @php($paymentPills = ['all','cash','digitally_paid','customer_wallet'])
        @php($currentPayment = $paymentMethod ?? 'all')
        <div class="card mb-3 report-card">
            <div class="card-body">
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col-12 col-lg-4">
                        <form action="#" id="form-data" method="GET" class="d-flex flex-wrap gap-2 justify-content-lg-end">
                ...
                <div class="status-filter mt-3">
                    @foreach($paymentPills as $method)
                        @php($pillQuery = ['payment_method'=>$method])
                        @if(request()->filled('searchValue'))
                            @php($pillQuery['searchValue'] = request('searchValue'))
                        @endif
                        @php($href = url()->current().'?'.http_build_query($pillQuery))
                        <a class="pill {{ $currentPayment === $method ? 'active' : '' }}" href="{{ $href }}">
                            {{ translate($method === 'digitally_paid' ? 'digitally_paid' : $method) }}
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div class="card">
            <div class="table-responsive">
                <table id="datatable"
                       class="text-start table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 __table-refund table-sticky">
                    ...
                    @foreach ($refundTransactions as $key=>$refund_transaction)
                        <tr class="text-capitalize">
                            ...
                            <td>
                                <span class="status-pill {{ 'status-' . $refund_transaction->payment_status }}">
