...
@section('content')
    <div class="content container-fluid">
        ...
        @if(request()->filled('searchValue'))
            @php($persistQuery['searchValue'] = request('searchValue'))
        @endif
        @if(request()->has('type'))
            @php($persistQuery['type'] = request('type'))
        @endif
        @php($persistQueryString = $persistQuery ? ('?'.http_build_query($persistQuery)) : '')
        @php($typeSelectBase = route('admin.refund-section.refund.list',['status'=>$status]))
        @php($searchCarry = request()->filled('searchValue') ? '&searchValue='.rawurlencode(request('searchValue')) : '')
        ...
        <div class="card mb-3 report-card">
            <div class="card-body">
                    ...
                    <div class="col-12 col-md-6 col-lg-8">
                        <div class="d-flex flex-wrap gap-3 justify-content-md-end">
                            ...
                            <select class="form-control w-auto"
                                    onchange="window.location='{{$typeSelectBase}}?type='+this.value+'{{$searchCarry}}'">
                                <option
                ...
                @php($statusFilters = ['pending','approved','refunded','rejected'])
                <div class="status-filter mt-3">
                    @foreach($statusFilters as $state)
                        @php($href = route('admin.refund-section.refund.list',['status'=>$state]).$persistQueryString)
                        <a class="pill {{ $status === $state ? 'active' : '' }}" href="{{ $href }}">
                            {{ translate($state) }}
                        </a>
                    @endforeach
                </div>
