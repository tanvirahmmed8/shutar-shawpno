        ...
        <div class="row mt-4">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="px-3 pb-2">
                        <div class="status-filter" aria-label="Quick vendor filters">
                            @php($baseVendorUrl = route('admin.vendors.vendor-list'))
                            <a href="{{ $baseVendorUrl }}" class="pill">{{ translate('All') }}</a>
                            <a href="{{ $baseVendorUrl }}?status=approved" class="pill pill-confirmed">{{ translate('Active') }}</a>
                            <a href="{{ $baseVendorUrl }}?status=inactive" class="pill pill-canceled">{{ translate('Inactive') }}</a>
                            <a href="{{ $baseVendorUrl }}?status=temporary_close" class="pill pill-processing">{{ translate('Temporary_Closed') }}</a>
                            <a href="{{ $baseVendorUrl }}?status=on_vacation" class="pill pill-out_for_delivery">{{ translate('On_Vacation') }}</a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table
                            style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                            class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 table-sticky">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-10 w-max-content">
                                            <img width="50"
                                            class="vendor-avatar avatar object-fit-cover" src="{{ getStorageImages(path: $seller?->shop?->image_full_url, type: 'backend-basic') }}"
                                                alt="">
                                            <div>
