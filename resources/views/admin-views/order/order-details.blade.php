...
@section('content')
    ...
    @php($orderPaidAmount = (float)($order->paid_amount ?? 0))
    @php($orderTotalAmount = (float)($order->order_amount ?? 0))
    @php($orderOutstandingAmount = max($orderTotalAmount - $orderPaidAmount, 0))
                ...
                <div class="card h-100">
                    <div class="card-body">
                            ...
                            <div class="text-sm-right flex-grow-1">
                                ...
                                <div class="d-flex flex-column gap-2 mt-3">
                                    ...
                                    <div class="payment-status d-flex justify-content-sm-end gap-10">
                                        ...
                                        @php($statusColor = $order['payment_status'] === 'paid' ? 'text-success' : ($order['payment_status'] === 'partial' ? 'text-warning' : 'text-danger'))
                                        <span class="{{$statusColor}} payment-status-span font-weight-bold">
                        ...
                        <div class="table-responsive datatable-custom">
                            <table
                                class="table fz-12 table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 table-sticky">
                ...
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <div class="d-flex justify-content-between align-items-center gap-10 form-control h-auto flex-wrap">
                            ...
                            <div class="d-flex justify-content-end min-w-100 align-items-center gap-2 flex-wrap">
                                <span class="text--primary font-weight-bold text-capitalize">
                                    {{ translate($order->payment_status) }}
                                </span>
                                <div class="min-w-150">
                                    @php($paymentStatuses = ['unpaid', 'partial', 'paid'])
                                    <select class="form-control payment-status payment-status-select" data-id="{{$order->id}}"
                                    ...
                                    @if($order->payment_status=='paid')
                                        <small class="text-muted d-block mt-1 payment-status-alert cursor-pointer">
                                            {{translate('when_payment_status_paid_then_you_can`t_change_payment_status_paid_to_unpaid').'.'}}
                                        </small>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-column gap-2 mb-4">
                                <div class="d-flex justify-content-between">
                                    <span class="title-color">{{ translate('order_amount') }}</span>
                                    <strong>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $orderTotalAmount), currencyCode: getCurrencyCode()) }}</strong>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="title-color">{{ translate('paid_amount') }}</span>
                                    <strong>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $orderPaidAmount), currencyCode: getCurrencyCode()) }}</strong>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="title-color">{{ translate('outstanding') }}</span>
                                    <strong class="{{ $orderOutstandingAmount > 0 ? 'text-danger' : 'text-success' }}">
                                        {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $orderOutstandingAmount), currencyCode: getCurrencyCode()) }}
                                    </strong>
                                </div>
                            </div>

                            @php($partialPaymentMethodOptions = $partialPaymentMethods ?? [])
                            @php($selectedPartialMethod = $partialPaymentSelectedMethod ?? 'cash')
                            @php($selectedPartialAccount = $partialPaymentSelectedAccount ?? 'cash')
                            @php($showCustomAccount = $selectedPartialAccount === 'custom')

                            @if($orderOutstandingAmount <= 0)
                                <div class="alert alert-success" role="alert">
                                    {{ translate('this_order_is_fully_paid') }}
                                </div>
                            @else
                                <form action="{{ route('admin.orders.partial-payments.store') }}" method="POST" class="partial-payment-form">
                                    @csrf
                                    <input type="hidden" name="order_id" value="{{ $order->id }}">
                                    <div class="form-group">
                                        <label class="title-color">{{ translate('amount') }}</label>
                                        <input type="number" min="0.01" step="0.01" max="{{ $orderOutstandingAmount }}" name="amount"
                                               class="form-control" value="{{ old('amount') }}"
                                               placeholder="{{ translate('enter_amount') }}" required>
                                        @error('amount')
                                            <span class="text-danger fz-12">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-group">
                                        <label class="title-color">{{ translate('payment_method') }}</label>
                                        <select name="payment_method" class="form-control partial-payment-method-select" required>
                                            @foreach($partialPaymentMethodOptions as $methodKey => $methodLabel)
                                                <option value="{{$methodKey}}" {{ $selectedPartialMethod == $methodKey ? 'selected' : '' }}>
                                                    {{ $methodLabel }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        <label class="title-color">{{ translate('payment_account') }}</label>
                                        <select name="payment_account" id="partial-payment-account-select" class="form-control partial-payment-account-select" data-selected="{{$selectedPartialAccount}}" required>
                                            @foreach($partialPaymentAccounts as $accountKey => $accountLabel)
                                                <option value="{{$accountKey}}" {{ $selectedPartialAccount == $accountKey ? 'selected' : '' }}>
                                                    {{ $accountLabel }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group {{ $showCustomAccount ? '' : 'd-none' }}" id="partial-payment-account-custom-wrapper">
                                        <label class="title-color">{{ translate('custom_account_name') }}</label>
                                        <input type="text" name="payment_account_custom" id="partial-payment-account-custom"
                                               class="form-control" value="{{ old('payment_account_custom') }}"
                                               placeholder="{{ translate('enter_account_name') }}">
                                        @error('payment_account_custom')
                                            <span class="text-danger fz-12">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-group">
                                        <label class="title-color">{{ translate('reference') }} ({{ translate('optional') }})</label>
                                        <input type="text" name="reference" class="form-control" value="{{ old('reference') }}"
                                               placeholder="{{ translate('e.g._transaction_id_or_receipt') }}">
                                        @error('reference')
                                            <span class="text-danger fz-12">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-group">
                                        <label class="title-color">{{ translate('note') }} ({{ translate('optional') }})</label>
                                        <textarea name="note" class="form-control" rows="2" placeholder="{{ translate('additional_details') }}">{{ old('note') }}</textarea>
                                        @error('note')
                                            <span class="text-danger fz-12">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-group">
                                        <label class="title-color">{{ translate('paid_at') }}</label>
                                    ...
                                    <button type="submit" class="btn btn--primary w-100 text-uppercase">{{ translate('add_partial_payment') }}</button>
                                </form>
                            @endif

                            @php($partialPayments = $order->partialPayments?->sortByDesc('paid_at') ?? collect())
                            <div class="mt-4">
                                <h5 class="title-color mb-3">{{ translate('payment_history') }}</h5>
                                @if($partialPayments->isEmpty())
                                    <div class="alert alert-light mb-0" role="alert">
                                        {{ translate('no_partial_payments_recorded_yet') }}
                                    </div>
                                @else
                                        ...
                                        <table class="table table-borderless table-sm">
                                            ...
                                            <tbody>
                                                @foreach($partialPayments as $partialPayment)
                                                    <tr>
                                                        <td>{{ optional($partialPayment->paid_at)->format('d M Y, h:i A') ?? '--' }}</td>
                                                        <td>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $partialPayment->amount), currencyCode: getCurrencyCode()) }}</td>
                                                        <td class="text-capitalize">{{ str_replace('_',' ', $partialPayment->payment_method ?? '-') }}</td>
                                                        <td>
                                                            {{ $partialPayment->payment_account }}
                                                            @if($partialPayment->payment_account_code)
                                                                <small class="text-muted">({{ $partialPayment->payment_account_code }})</small>
                                                            @endif
                                                        </td>
                                                        <td>{{ $partialPayment->reference ?? '--' }}</td>
                                                    </tr>
                                                    @if($partialPayment->note)
                                                        <tr>
                                                            <td colspan="5" class="bg-light text-muted">
                                                                <small>{{ translate('note') }}:</small>
                                                                <small>{{ $partialPayment->note }}</small>
                                                            </td>
                                                        </tr>
                                                    @endif
                                                @endforeach
    ...
    <span id="partial-payment-account-matrix" data-matrix='@json($partialPaymentMatrix ?? [])'></span>
