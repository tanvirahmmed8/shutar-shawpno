        ...
        <div>
            ...
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    <div class="px-3 pb-3">
                        <div class="status-filter" aria-label="Quick status filters">
                            <a href="{{ route('admin.orders.list',['all']) }}" class="pill pill-all">{{ translate('All') }}</a>
                            <a href="{{ route('admin.orders.list',['pending']) }}" class="pill pill-pending">{{ translate('Pending') }}</a>
                            <a href="{{ route('admin.orders.list',['confirmed']) }}" class="pill pill-confirmed">{{ translate('Confirmed') }}</a>
                            <a href="{{ route('admin.orders.list',['processing']) }}" class="pill pill-processing">{{ translate('Packaging') }}</a>
                            <a href="{{ route('admin.orders.list',['out_for_delivery']) }}" class="pill pill-out_for_delivery">{{ translate('Out_for_delivery') }}</a>
                            <a href="{{ route('admin.orders.list',['delivered']) }}" class="pill pill-delivered">{{ translate('Delivered') }}</a>
                            <a href="{{ route('admin.orders.list',['returned']) }}" class="pill pill-returned">{{ translate('Returned') }}</a>
                            <a href="{{ route('admin.orders.list',['failed']) }}" class="pill pill-failed">{{ translate('Failed') }}</a>
                            <a href="{{ route('admin.orders.list',['canceled']) }}" class="pill pill-canceled">{{ translate('Canceled') }}</a>
                        </div>
                    </div>
                    <div class="table-responsive datatable-custom">
                        <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 text-start table-sticky">
                                    ...
                                    @if($status == 'all')
                                        <td class="text-center text-capitalize">
                                            @if($order['order_status']=='pending')
                                                <span class="status-pill status-pending fz-12">
                                                    {{translate($order['order_status'])}}
                                                </span>
                                            @elseif($order['order_status']=='processing' || $order['order_status']=='out_for_delivery')
                                                <span class="status-pill status-processing fz-12">
                                                    {{str_replace('_',' ',$order['order_status'] == 'processing' ? translate('packaging'):translate($order['order_status']))}}
                                                </span>
                                            @elseif($order['order_status']=='confirmed')
                                                <span class="status-pill status-confirmed fz-12">
                                                    {{translate($order['order_status'])}}
                                                </span>
                                            @elseif($order['order_status']=='failed')
                                                <span class="status-pill status-failed fz-12">
                                                    {{translate('failed_to_deliver')}}
                                                </span>
                                            @elseif($order['order_status']=='delivered')
                                                <span class="status-pill status-delivered fz-12">
                                                    {{translate($order['order_status'])}}
                                                </span>
                                            @else
                                                <span class="status-pill status-{{$order['order_status']}} fz-12">
                                                    {{translate($order['order_status'])}}
                                                </span>
                                            @endif
                                        </td>
