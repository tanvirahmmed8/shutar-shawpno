    ...
    <div class="content container-fluid">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
            ...
            <button class="btn-how-it-works d-flex align-items-center gap-2 font-weight-medium text-capitalize cursor-pointer"
                ...
                <span class="opacity--80">{{ translate('how_it_work') }}</span>
            </button>
        </div>
        ...
        @php($analyticsCards = [
            [
                ...
                'title' => translate('Google_Analytics'),
                'label' => translate('Google_Analytics_Measurement_ID'),
                'placeholder' => translate('Enter_the_GA_Measurement_ID'),
                'modal' => '#modalForGoogleAnalytics',
            ],
            [
                'key' => 'google_tag_manager',
                'title' => translate('Google_Tag_Manager'),
                'label' => translate('Google_Tag_Manager_Container_ID'),
                'placeholder' => translate('enter_the_GTM_Container_ID'),
                'modal' => '#modalForGoogleTagManager',
            ],
            [
                'key' => 'linkedin_insight',
                'title' => translate('LinkedIn_Insight_Tag'),
                'label' => translate('LinkedIn_Partner_ID'),
                'placeholder' => translate('Enter_the_LinkedIn_Partner_ID'),
                'modal' => '#modalForLinkedInInsight',
            ],
            [
                'key' => 'meta_pixel',
                'title' => translate('Meta_Pixel'),
                'label' => translate('Meta_Pixel_ID'),
                'placeholder' => translate('Enter_the_Meta_Pixel_ID'),
                'modal' => '#modalForFacebookMeta',
            ],
            [
                'key' => 'pinterest_tag',
                'title' => translate('Pinterest_Pixel'),
                'label' => translate('Pinterest_Tag_ID'),
                'placeholder' => translate('Enter_the_Pinterest_Tag_ID'),
                'modal' => '#modalForPinterestPixel',
            ],
            [
                'key' => 'snapchat_tag',
                'title' => translate('Snapchat_Pixel'),
                'label' => translate('Snap_Pixel_ID'),
                'placeholder' => translate('Enter_the_Snap_Pixel_ID'),
                'modal' => '#modalForSnapchatPixel',
            ],
            [
                'key' => 'tiktok_tag',
                'title' => translate('TikTok_Pixel'),
                'label' => translate('TikTok_Pixel_ID'),
                'placeholder' => translate('Enter_the_TikTok_Pixel_ID'),
                'modal' => '#modalForTikTokPixel',
            ],
            [
                'key' => 'twitter_tag',
                'title' => translate('X').' ('.translate('Twitter').') '.translate('Pixel'),
                'label' => translate('Pixel_ID'),
                'placeholder' => translate('Enter_the_Pixel_ID'),
        ...
        <div class="row g-3 g-lg-4">
            @foreach($analyticsCards as $card)
                @php($config = $analyticsData[$card['key']] ?? null)
                @php($isActive = $config?->is_active == 1)
                <div class="col-lg-6">
                    <div class="card report-card h-100">
                        <form action="{{ env('APP_MODE') != 'demo' ? route('admin.business-settings.analytics-update') : 'javascript:' }}"
                              method="post">
                            @csrf
                            <div class="card-header border-0 pb-0">
                                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                                    <h5 class="mb-0 text-capitalize">{{ $card['title'] }}</h5>
                                    <div class="d-flex align-items-center gap-3">
                                        <span class="status-pill {{ $isActive ? 'status-pill--success' : 'status-pill--danger' }}">
                                            {{ $isActive ? translate('enabled') : translate('disabled') }}
                                        </span>
                                        <label class="switcher mb-0">
                                            <input type="checkbox" name="is_active" value="1" class="switcher_input" {{ $isActive ? 'checked' : '' }}>
                            ...
                            <div class="card-body d-flex flex-column gap-4">
                                <div class="form-group mb-0">
                                    <label class="title-color d-flex font-weight-medium">{{ $card['label'] }}</label>
                                    <input type="hidden" name="type" value="{{ $card['key'] }}">
                                    <textarea class="form-control" rows="2" placeholder="{{ $card['placeholder'] }}" name="script_id">{{ $config?->script_id ?? '' }}</textarea>
                                </div>
                                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                                    <button type="button"
                                            class="btn btn-link px-0 text-capitalize font-weight-medium d-flex align-items-center gap-2"
