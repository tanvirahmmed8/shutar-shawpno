...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="card report-card mb-3">
            @php($isMaintenanceOn = (bool) $businessSetting['maintenance_mode'])
            <div class="card-header border-0 pb-0">
                <div class="d-flex flex-wrap align-items-center gap-3 justify-content-between">
                    <div class="d-flex flex-column">
                        <h5 class="mb-1 text-capitalize d-flex align-items-center gap-2">
                            <i class="tio-notifications-alert mr-1"></i>
                            {{ translate('System_Maintenance') }}
                        </h5>
                        <p class="text-muted mb-0">{{translate('keep_users_informed_about_planned_downtime_and_resume_services_confidently')}}</p>
                    </div>
                    <span class="status-pill {{ $isMaintenanceOn ? 'status-pill--danger' : 'status-pill--success' }}">
                        {{ $isMaintenanceOn ? translate('enabled') : translate('disabled') }}
                    </span>
                </div>
            </div>
            <div class="card-body">
                <div class="row g-3 align-items-start">
                        ...
                        <div class="d-flex flex-column gap-3">
                                ...
                                <div class="d-flex flex-wrap gap-3 align-items-center">
                                    <p class="mb-0">
                                        ...
                                        @if($selectedMaintenanceDuration['maintenance_duration'] != 'until_change')
                                            {{ translate('from ') }}<strong>{{ $maintenanceStartDate->format('m/d/Y, h:i A') }}</strong> {{ translate('to') }} <strong>{{ $maintenanceEndDate->format('m/d/Y, h:i A') }}</strong>.
                                        @endif
                                    </p>
                                    <a class="btn btn-outline-primary btn-sm edit square-btn maintenance-mode-show" href="#"><i class="tio-edit"></i></a>
                                </div>
                            @else
                                <p class="m-0 text-muted">{{ translate('By turning on maintenance mode Control your all system & function') }}</p>
                            @endif
                            ...
                            @if($businessSetting['maintenance_mode'] && count($maintenanceSystemSetup) > 0)
                                ...
                                <div class="d-flex flex-column gap-2">
                                    <h5 class="mb-0">{{ translate('Selected_Systems') }}</h5>
                                    <div class="bg-soft-dark rounded px-4 py-2">
                                        <ul class="selected-systems d-flex flex-wrap gap-3 mb-0">
                    ...
                    <div class="col-md-4 col-xl-3">
                        <div class="border rounded px-4 py-3 h-100 d-flex flex-column gap-3">
                            ...
                            <p class="text-muted small mb-0">{{translate('toggle_anytime_to_pause_customer-facing_surfaces_with_confidence')}}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-3 report-card">
                <div class="card-header border-0 pb-0">
                    <div class="d-flex flex-column gap-1">
                        ...
                        <p class="text-muted mb-0">{{translate('keep_primary_contact_details_current_to_power_notifications_and_invoices')}}</p>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row g-3">
                            ...
                            <div class="col-12">
                                <div class="form-group mb-0">
                                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                        <div class="badge badge--primary-2">
                                            {{translate('latitude')}}:
                                            <span id="showLatitude">{{($default_location['lat']??'-33.8688')}}</span>
                                        </div>
                                        <div class="badge badge--primary-2">
                                            {{translate('longitude')}}:
                                            <span id="showLongitude">{{($default_location['lng']??'151.2195')}}</span>
                                        </div>
                                    </div>
                                    <input id="map-pac-input" class="form-control rounded __map-input mt-3"
                                           title="{{translate('search_your_location_here')}}" type="text"
                                           placeholder="{{translate('search_here')}}"/>
                                    <div class="rounded w-100 __h-200px mt-3"
            ...
            <div class="card mb-3 report-card">
                <div class="card-header border-0 pb-0">
                    <div class="d-flex flex-column gap-1">
                        ...
                        <p class="text-muted mb-0">{{translate('currency_behavior_and_listing_density_live_here_update_carefully')}}</p>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row align-items-end g-3">
                        ...
                        <div class="col-sm-6 col-lg-4">
                            ...
                            <div class="form-control min-form-control-height form-group d-flex gap-2 flex-wrap">
            ...
            <div class="card mb-4 report-card">
                <div class="card-header border-0 pb-0">
                    <div class="d-flex flex-column gap-1">
                        ...
                        <p class="text-muted mb-0">{{translate('control_where_app_store_badges_show_up_and_keep_links_fresh')}}</p>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row g-4">
                        <div class="col-lg-6">
                            ...
                            <div class="bg-aliceblue p-3 rounded h-100">
            ...
            <div class="row g-3">
                <div class="col-xxl-4 col-sm-6">
                    <div class="card h-100 report-card">
                        ...
                        <div class="card-body">
                            <div class="row g-3 justify-content-center text-center text-sm-left">
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        <input type="color" name="primary" value="{{ $businessSetting['primary_color'] }}"
                                               class="form-control form-control_color">
                                        <div class="mt-3">
                                            <div class="title-color mb-2">{{ strtoupper($businessSetting['primary_color']) }}</div>
                                            <label class="title-color text-capitalize mb-0">{{translate('primary_Color')}}</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        <input type="color" name="secondary" value="{{ $businessSetting['secondary_color'] }}"
                                               class="form-control form-control_color">
                                        <div class="mt-3">
                                            <div class="title-color mb-2">{{ strtoupper($businessSetting['secondary_color']) }}</div>
                                            <label class="title-color text-capitalize mb-0">{{translate('secondary_Color')}}</label>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group mb-0">
                                            <input type="color" name="primary_light"
                                                   ...
                                                   class="form-control form-control_color">
                                            <div class="mt-3">
                                                <div class="title-color mb-2">{{ $businessSetting['primary_color_light'] ?? '#CFDFFB' }}</div>
                                                <label class="title-color text-capitalize mb-0">{{translate('primary_Light_Color')}}</label>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                ...
                <div class="col-xxl-4 col-sm-6">
                    <div class="card h-100 report-card">
                        ...
                        <div class="card-body d-flex flex-column justify-content-around gap-3">
