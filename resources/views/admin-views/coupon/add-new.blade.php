            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="px-3 pb-2 d-flex flex-wrap gap-2 align-items-center justify-content-between">
                        <div class="status-filter" aria-label="Quick coupon filters">
                            @php($baseCouponUrl = route('admin.coupon.add'))
                            <a href="{{ $baseCouponUrl }}" class="pill">{{ translate('All') }}</a>
                            <a href="{{ $baseCouponUrl }}?status=1" class="pill pill-confirmed">{{ translate('Active') }}</a>
                            <a href="{{ $baseCouponUrl }}?status=0" class="pill pill-canceled">{{ translate('Inactive') }}</a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table id="datatable"
                               class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table table-sticky {{ Session::get('direction') === 'rtl' ? 'text-right' : 'text-left' }}">
