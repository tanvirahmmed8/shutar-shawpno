    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-2 report-card">
            <div class="card-body">
                ...
                <div class="mt-3 status-filter">
                    <a class="pill {{ $date_type=='today' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['seller_id'=>request('seller_id'),'date_type'=>'today','from'=>request('from'),'to'=>request('to'),'search'=>request('search')]) }}">{{ translate('today') }}</a>
                    <a class="pill {{ $date_type=='this_week' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['seller_id'=>request('seller_id'),'date_type'=>'this_week','from'=>request('from'),'to'=>request('to'),'search'=>request('search')]) }}">{{ translate('this_Week') }}</a>
                    <a class="pill {{ $date_type=='this_month' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['seller_id'=>request('seller_id'),'date_type'=>'this_month','from'=>request('from'),'to'=>request('to'),'search'=>request('search')]) }}">{{ translate('this_Month') }}</a>
                    <a class="pill {{ $date_type=='this_year' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['seller_id'=>request('seller_id'),'date_type'=>'this_year','from'=>request('from'),'to'=>request('to'),'search'=>request('search')]) }}">{{ translate('this_Year') }}</a>
                    <a class="pill {{ $date_type=='custom_date' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['seller_id'=>request('seller_id'),'date_type'=>'custom_date','from'=>request('from'),'to'=>request('to'),'search'=>request('search')]) }}">{{ translate('custom_Date') }}</a>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="table-responsive">
                <table id="datatable"
                       class="table __table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 table-sticky">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center">
                                    @php($statusClass = $order['order_status'] == 'processing' ? 'processing' : $order['order_status'])
...
@push('script_2')
    ...
    <script src="{{ dynamicAsset(path: 'public/site-assets/back-end/js/admin/order-report.js') }}"></script>
@endpush
