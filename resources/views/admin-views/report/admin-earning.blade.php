        ...
        <div class="card mb-2 report-card">
            <div class="card-body">
                ...
                <div class="mt-3 status-filter">
                    <a class="pill {{ $date_type=='today' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['date_type'=>'today','from'=>request('from'),'to'=>request('to')]) }}">{{ translate('today') }}</a>
                    <a class="pill {{ $date_type=='this_week' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['date_type'=>'this_week','from'=>request('from'),'to'=>request('to')]) }}">{{ translate('this_Week') }}</a>
                    <a class="pill {{ $date_type=='this_month' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['date_type'=>'this_month','from'=>request('from'),'to'=>request('to')]) }}">{{ translate('this_Month') }}</a>
                    <a class="pill {{ $date_type=='this_year' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['date_type'=>'this_year','from'=>request('from'),'to'=>request('to')]) }}">{{ translate('this_Year') }}</a>
                    <a class="pill {{ $date_type=='custom_date' ? 'active' : '' }}" href="{{ url()->current() }}?{{ http_build_query(['date_type'=>'custom_date','from'=>request('from'),'to'=>request('to')]) }}">{{ translate('custom_Date') }}</a>
                </div>
            </div>
        </div>
...
@push('script_2')
    ...
    <script src="{{ dynamicAsset(path: 'public/site-assets/back-end/js/admin/admin-earning-report.js') }}"></script>
@endpush
