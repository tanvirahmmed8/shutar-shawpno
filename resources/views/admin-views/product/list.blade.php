                ...
                <div class="card">
                    ...
                    <div class="px-3 pb-2">
                        <div class="status-filter" aria-label="Quick status filters">
                            @php($baseUrl = route('admin.products.list',['type'=>request('type')]))
                            <a href="{{ $baseUrl }}" class="pill">{{ translate('All') }}</a>
                            <a href="{{ $baseUrl }}?status=active" class="pill pill-confirmed">{{ translate('Active') }}</a>
                            <a href="{{ $baseUrl }}?status=inactive" class="pill pill-canceled">{{ translate('Inactive') }}</a>
                            <a href="{{ $baseUrl }}?status=featured" class="pill pill-processing">{{ translate('Featured') }}</a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table id="datatable"
                               class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 text-start table-sticky">
                                        ...
                                        <a href="{{ route('admin.products.view',['addedBy'=>($product['added_by']=='seller'?'vendor' : 'in-house'),'id'=>$product['id']]) }}"
                                           class="media align-items-center gap-2">
                                            <img src="{{ getStorageImages(path: $product->thumbnail_full_url, type: 'backend-product') }}"
                                                 class="avatar border prod-avatar" alt="">
                                            <div>
                                                <div class="media-body title-color hover-c1">
                                                    {{ Str::limit($product['name'], 20) }}
                                                </div>
                                                @if($product?->clearanceSale)
                                                    <div class="badge-featured user-select-none">
