...
<head>
    ...
    <meta name="description" content="{{ $web_config['meta_description'] ?? Str::limit(strip_tags($web_config['company_name'].' - '.$web_config['about'] ?? ''), 160) }}">
    <meta property="og:title" content="@yield('title', $web_config['company_name'])" />
    <meta property="og:description" content="{{ $web_config['meta_description'] ?? Str::limit(strip_tags($web_config['company_name'].' - '.$web_config['about'] ?? ''), 160) }}" />
    <meta property="og:image" content="{{ $web_config['og_image']['path'] ?? $web_config['web_logo']['path'] ?? '' }}" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="@yield('title', $web_config['company_name'])" />
    <meta name="twitter:description" content="{{ $web_config['meta_description'] ?? Str::limit(strip_tags($web_config['company_name'].' - '.$web_config['about'] ?? ''), 160) }}" />
    <meta name="theme-color" content="{{ $web_config['primary_color'] }}">
    ...
    <style>
        :root {
            /* Original system colors - preserved for compatibility */
            ...
            /* Fashion Brand Integration - Override with brand colors if needed */
            --fashion-primary: {{ $web_config['primary_color'] ?? '#1a1a1a' }};
            --fashion-secondary: {{ $web_config['secondary_color'] ?? '#d4af37' }};
        }
    ...
    <style>
        :root {
            /* Map brand tokens from backend config */
            --brand-primary: {{ $web_config['primary_color'] ?? '#2b6cb0' }};
            --brand-secondary: {{ $web_config['secondary_color'] ?? '#805ad5' }};
...
<a href="#main-content" class="visually-hidden-focusable">{{ translate('Skip to main content') }}</a>
...
<main id="main-content" role="main" tabindex="-1" class="fashion-main">
@yield('content')
</main>
