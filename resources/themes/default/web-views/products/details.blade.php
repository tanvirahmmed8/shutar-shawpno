...
@section('content')
    <div class="fashion-product-detail-page __inline-23">
        <div class="fashion-container container mt-4 rtl text-align-direction">
            <div class="fashion-product-layout row {{Session::get('direction') === "rtl" ? '__dir-rtl' : ''}}">
                <div class="fashion-product-main col-lg-9">
                    <div class="fashion-product-content row">
                        <div class="fashion-product-gallery-section col-lg-5 col-md-4">
                            <div class="fashion-product-gallery cz-product-gallery">
                                <div class="fashion-gallery-preview cz-preview">
                                    <div id="sync1" class="fashion-main-slider owl-carousel owl-theme product-thumbnail-slider">
                                        @if($product->images!=null && json_decode($product->images)>0)
                                            @if(json_decode($product->colors) && count($product->color_images_full_url)>0)
                                                @foreach ($product->color_images_full_url as $key => $photo)
                                                    @if($photo['color'] != null)
                                                        <div
                                                            class="fashion-preview-item product-preview-item pd-gallery-main d-flex align-items-center justify-content-center {{$key==0?'active':''}}"
                                                            id="image{{$photo['color']}}">
                                                            <img class="fashion-main-image cz-image-zoom img-responsive w-100"
                                                                 src="{{ getStorageImages(path: $photo['image_name'], type: 'product') }}"
                                                                 data-zoom="{{ getStorageImages(path: $photo['image_name'], type: 'product')  }}"
                                                                 alt="{{ $product->name }}" width="" loading="{{$key==0 ? 'eager' : 'lazy'}}" fetchpriority="{{$key==0 ? 'high' : 'auto'}}" decoding="async">
                                                            <div class="fashion-zoom-pane cz-image-zoom-pane"></div>
                                                        </div>
                                                    @else
                                                        <div
                                                            class="product-preview-item pd-gallery-main d-flex align-items-center justify-content-center {{$key==0?'active':''}}"
                                                            id="image{{$key}}">
                                                            <img class="cz-image-zoom img-responsive w-100"
                                                                 src="{{ getStorageImages(path: $photo['image_name'], type: 'product') }}"
                                                                 data-zoom="{{ getStorageImages(path: $photo['image_name'], type: 'product') }}"
                                                                 alt="{{ translate('product') }}" width="" loading="{{$key==0 ? 'eager' : 'lazy'}}" fetchpriority="{{$key==0 ? 'high' : 'auto'}}" decoding="async">
                                                ...
                                                @if($product->images!=null && json_decode($product->images)>0)
                                                    @if(json_decode($product->colors) && count($product->color_images_full_url)>0)
                                                        @foreach ($product->color_images_full_url as $key => $photo)
                                                            @if($photo['color'] != null)
                                                                <div class="">
                                                       <a class="product-preview-thumb pd-thumb color-variants-preview-box-{{ $photo['color'] }} {{$key==0?'active':''}} d-flex align-items-center justify-content-center"
                                                                       id="preview-img{{$photo['color']}}"
                                                                       href="#image{{$photo['color']}}">
                                                          <img alt="{{ translate('product') }}"
                                                              src="{{ getStorageImages(path: $photo['image_name'], type: 'product') }}" loading="lazy" decoding="async">
                                                                    </a>
                                                                </div>
                                                            @else
                                                                <div class="">
                                                       <a class="product-preview-thumb pd-thumb {{$key==0?'active':''}} d-flex align-items-center justify-content-center"
                                                    ...
                                                    <a class="product-preview-thumb pd-thumb {{$key==0?'active':''}} d-flex align-items-center justify-content-center"
                                                                   id="preview-img{{$key}}" href="#image{{$key}}">
                                                       <img alt="{{ translate('product') }}"
                                                           src="{{ getStorageImages(path: $photo, type: 'product') }}" loading="lazy" decoding="async">
                                                                </a>
                                                            </div>
                                                        @endforeach
                                                    @endif
                        ...
                        <div class="fashion-product-info-section col-lg-7 col-md-8 mt-md-0 mt-sm-3 web-direction">
                            <div class="fashion-product-details details __h-100 product-cart-option-container">
                                <h1 class="fashion-product-title mb-2 __inline-24">{{ $product->name }}</h1>
                                <div class="fashion-product-meta d-flex flex-wrap align-items-center mb-2 pro">
                                    <div class="fashion-rating-display star-rating me-2">
                                        @for($inc=1;$inc<=5;$inc++)
                                            @if ($inc <= (int)$overallRating[0])
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="fashion-star-filled">
                                            ...
                                            @elseif ($overallRating[0] != 0 && $inc <= (int)$overallRating[0] + 1.1 && $overallRating[0] > ((int)$overallRating[0]))
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="fashion-star-half">
                                                    <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="url(#halfStarDetail)"/>
                                                    <defs>
                                            ...
                                            @else
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="fashion-star-empty">
                                    ...
                                    <span class="fashion-rating-score d-inline-block align-middle mt-1 {{Session::get('direction') === "rtl" ? 'ml-md-2 ml-sm-0' : 'mr-md-2 mr-sm-0'}} fs-14 text-muted">({{$overallRating[0]}})</span>
                                    <span class="fashion-review-count font-regular font-for-tab d-inline-block font-size-sm text-body align-middle mt-1 {{Session::get('direction') === "rtl" ? 'mr-1 ml-md-2 ml-1 pr-md-2 pr-sm-1 pl-md-2 pl-sm-1' : 'ml-1 mr-md-2 mr-1 pl-md-2 pl-sm-1 pr-md-2 pr-sm-1'}}"><span class="web-text-primary">{{$overallRating[1]}}</span> {{translate('reviews')}}</span>
                                    <span class="fashion-meta-separator __inline-25"></span>
                                    <span class="fashion-order-count font-regular font-for-tab d-inline-block font-size-sm text-body align-middle mt-1 {{Session::get('direction') === "rtl" ? 'mr-1 ml-md-2 ml-1 pr-md-2 pr-sm-1 pl-md-2 pl-sm-1' : 'ml-1 mr-md-2 mr-1 pl-md-2 pl-sm-1 pr-md-2 pr-sm-1'}}"><span class="web-text-primary">{{$countOrder}}</span> {{translate('orders')}}</span>
                                    <span class="fashion-meta-separator __inline-25"></span>
                                    <span class="fashion-wishlist-count font-regular font-for-tab d-inline-block font-size-sm text-body align-middle mt-1 {{Session::get('direction') === "rtl" ? 'mr-1 ml-md-2 ml-0 pr-md-2 pr-sm-1 pl-md-2 pl-sm-1' : 'ml-1 mr-md-2 mr-0 pl-md-2 pl-sm-1 pr-md-2 pr-sm-1'}} text-capitalize"><span class="web-text-primary countWishlist-{{ $product->id }}">{{$countWishlist}}</span> {{translate('wish_listed')}}</span>
                                </div>
                                ...
                                <form class="fashion-product-form mb-2 addToCartDynamicForm add-to-cart-details-form">

                                    <div class="fashion-pricing-section mb-3">
                                        <span class="fashion-price-container font-weight-normal text-accent d-flex align-items-end gap-2">
                                            <span class="fashion-current-price discounted-unit-price fs-24 font-bold">
                                    ...
                                    @foreach (json_decode($product->choice_options) as $key => $choice)
                                        <div class="fashion-variant-group row flex-start mx-0 align-items-center">
                                            <div class="fashion-variant-label product-description-label text-dark font-bold {{Session::get('direction') === "rtl" ? 'pl-2' : 'pr-2'}} text-capitalize mb-2">{{ $choice->title }}:</div>
                                            <div class="fashion-variant-options">
                                                <div class="fashion-options-list list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 mx-1 flex-start row ps-0">
                                                    @foreach ($choice->options as $index => $option)
                                                        <div class="fashion-option-item user-select-none">
                                                            <div class="fashion-option-wrapper for-mobile-capacity">
                                                                <input type="radio"
                                                                       id="{{ str_replace(' ', '', ($choice->name. '-'. $option)) }}"
                                                                       name="{{ $choice->name }}" value="{{ $option }}"
                                                                       @if($index == 0) checked @endif >
                                                                <label class="fashion-option-label __text-12px"
                                    ...
                                    <div class="fashion-quantity-section mt-3">
                                        <div class="fashion-quantity-container product-quantity d-flex flex-column __gap-15">
                                            <div class="fashion-quantity-controls d-flex align-items-center gap-3">
                                                <div class="fashion-quantity-label product-description-label text-dark font-bold mt-0">
                                                    {{translate('quantity')}}:
                                                </div>
                                                <div class="fashion-quantity-input d-flex justify-content-center align-items-center quantity-box border rounded border-base web-text-primary">
                                                    <span class="fashion-quantity-btn input-group-btn">
                                                        <button class="fashion-btn-minus btn btn-number __p-10 web-text-primary" type="button"
                                                    ...
                                                    <input type="text" name="quantity"
                                                           class="fashion-quantity-field form-control input-number text-center product-details-cart-qty __inline-29 border-0"
                                                    ...
                                                    <span class="fashion-quantity-btn input-group-btn">
                                                        <button class="fashion-btn-plus btn btn-number __p-10 web-text-primary" type="button"
                                                                data-producttype="{{ $product->product_type }}"
                                    ...
                                    <div class="fashion-action-buttons __btn-grp mt-2 mb-3 product-add-and-buy-section-parent">
                                        <div class="fashion-purchase-buttons product-add-and-buy-section gap-2 {!! $firstVariationQuantity <= 0 ? '' : 'd-flex' !!}" {!! $firstVariationQuantity <= 0 ? 'style="display: none;"' : '' !!}>
                                            @if(($product->added_by == 'seller' && ($sellerTemporaryClose || (isset($product->seller->shop) && $product->seller->shop->vacation_status && $currentDate >= $sellerVacationStartDate && $currentDate <= $sellerVacationEndDate))) ||
                                             ($product->added_by == 'admin' && ($inHouseTemporaryClose || ($inHouseVacationStatus && $currentDate >= $inHouseVacationStartDate && $currentDate <= $inHouseVacationEndDate))))
                                                    <button class="fashion-btn-disabled btn btn-secondary" type="button" disabled>
                                                        {{ translate('buy_now') }}
                                                    </button>
                                                    <button class="fashion-btn-disabled btn btn--primary string-limit" type="button" disabled>
                                                        {{ translate('add_to_cart') }}
                                                    </button>
                                                @else
                                                    <button type="button"
                                                            class="fashion-buy-now-btn btn btn-secondary element-center btn-gap-{{Session::get('direction') === "rtl" ? 'left' : 'right'}} product-buy-now-button"
                                                    ...
                                                    >
                                                        <span class="fashion-btn-text string-limit">{{ translate('buy_now') }}</span>
                                                    </button>
                                                    <button class="fashion-add-to-cart-btn btn btn--primary element-center product-add-to-cart-button"
                                                            type="button"
                                                            data-form=".add-to-cart-details-form"
                                                            data-update="{{ translate('update_cart') }}"
                                                            data-add="{{ translate('add_to_cart') }}"
                                                    >
                                                        <span class="fashion-btn-text string-limit">{{ translate('add_to_cart') }}</span>
                                                    </button>
                                                @endif
                                        </div>
                                        ...
                                        @endif
                                        <button type="button" data-product-id="{{ $product['id'] }}" class="fashion-wishlist-btn btn __text-18px border product-action-add-wishlist">
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="fashion-heart-icon wishlist_icon_{{$product['id']}} {{$wishlistStatus == 1 ? 'fashion-heart-filled' : 'fashion-heart-empty'}}">
                                                <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke="currentColor" stroke-width="2" fill="{{$wishlistStatus == 1 ? 'currentColor' : 'none'}}"/>
                                            </svg>
                                            <span class="fashion-wishlist-count fs-14 text-muted align-bottom countWishlist-{{$product['id']}}">{{$countWishlist}}</span>
                                            <div class="fashion-wishlist-tooltip wishlist-tooltip" x-placement="top">
                                                <div class="arrow"></div><div class="inner">
                                                    <span class="add">{{translate('added_to_wishlist')}}</span>
                                                    <span class="remove">{{translate('removed_from_wishlist')}}</span>
                                                </div>
                    ...
                    <div class="fashion-product-tabs mt-4 rtl text-align-direction">
                        <div class="fashion-tabs-container px-4 pb-3 mb-3 mr-0 mr-md-2 bg-white __review-overview __rounded-10 pt-3">
                            <ul class="fashion-nav-tabs nav nav-tabs nav--tabs d-flex justify-content-center mt-3"
                                role="tablist">
                                <li class="fashion-nav-item nav-item">
                                    <a class="fashion-nav-link nav-link __inline-27 active" href="#overview"
                                ...
                                <li class="fashion-nav-item nav-item">
                                    <a class="fashion-nav-link nav-link __inline-27" href="#reviews" data-toggle="tab"
                                        role="tab">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="me-2">
