...
class OrderController extends BaseController
{
    ...
    public function getView(string|int $id, DeliveryCountryCodeService $service, OrderService $orderService): View|RedirectResponse
    {
        ...
        $order = $this->orderRepo->getFirstWhere(params: ['id' => $id], relations: ['details.productAllStatus', 'details.lotAllocations.lot', 'verificationImages', 'shipping', 'seller.shop', 'offlinePayments', 'deliveryMan', 'partialPayments']);
        ...
        if ($order) {
            ...
            $partialPaymentSelectedMethod = PaymentAccountMapper::normalizeMethod(old('payment_method', $order['payment_method'] ?? 'cash'));
            $partialPaymentMethods = PaymentAccountMapper::methodOptions();
            $partialPaymentAccounts = PaymentAccountMapper::accountOptionsFor($partialPaymentSelectedMethod);
            $partialPaymentSelectedAccount = old('payment_account', PaymentAccountMapper::defaultAccountFor($partialPaymentSelectedMethod));
            $partialPaymentMatrix = PaymentAccountMapper::methodAccountMatrixForJs();
            ...
            if ($order['order_type'] == 'default_type') {
                ...
                return view(Order::VIEW[VIEW], compact('order', 'linkedOrders',
                    ...
                    'countryRestrictStatus', 'zipRestrictStatus', 'countries', 'zipCodes', 'orderCount', 'isOrderOnlyDigital', 'partialPaymentAccounts', 'partialPaymentMethods', 'partialPaymentSelectedMethod', 'partialPaymentSelectedAccount', 'partialPaymentMatrix'));
            } else {
    ...
    public function storePartialPayment(Request $request): RedirectResponse
    {
        ...
        $request->validate([
            ...
            'payment_account_custom' => ['required_if:payment_account,custom', 'nullable', 'string', 'max:191'],
        ...
        $orderTotal = (float)($order['order_amount'] ?? 0);
        $outstanding = max($orderTotal - $currentPaid, 0);
        $amount = (float)$request['amount'];

        if ($amount > $outstanding) {
            Toastr::error(translate('partial_payment_can_not_exceed_outstanding_amount'));
            return back()->withInput();
        }

        $normalizedMethod = PaymentAccountMapper::normalizeMethod($request['payment_method']);
        $accountSelection = $request['payment_account'];
        $resolvedAccount = null;

        if ($accountSelection !== 'custom') {
            $resolvedAccount = PaymentAccountMapper::resolveAccount($normalizedMethod, $accountSelection);
            if (empty($resolvedAccount)) {
                return back()->withErrors(['payment_account' => translate('selected_payment_account_is_not_available_for_this_method')])->withInput();
            }
        } elseif (!filled($request['payment_account_custom'])) {
            return back()->withErrors(['payment_account_custom' => translate('please_specify_the_payment_account')])->withInput();
        }

        DB::transaction(function () use ($order, $amount, $resolvedAccount, $request, $currentPaid, $orderTotal) {
            OrderPartialPayment::create([
                'order_id' => $order['id'],
                'amount' => $amount,
                'payment_method' => $request['payment_method'],
                'payment_account' => $resolvedAccount['label'] ?? $request['payment_account_custom'],
                'payment_account_code' => $resolvedAccount['code'] ?? null,
            ...
            $newStatus = 'unpaid';
            if ($newPaidTotal >= $orderTotal && $orderTotal > 0) {
                $newStatus = 'paid';
        ...
        Toastr::success(translate('partial_payment_recorded_successfully'));
        return back();
    }
