...
class RefundController extends BaseController
{
    ...
    public function updateRefundStatus(RefundStatusRequest $request, RefundStatusService $refundStatusService, RefundTransactionService $refundTransactionService): JsonResponse
    {
        ...
        if ($refund['status'] != 'refunded') {
            ...
            if ($request['refund_status'] == 'refunded' && app()->bound(FinancePostingService::class)) {
                $updatedRefund = $this->refundRequestRepo->getFirstWhere(
                    params: ['id' => $refund['id']],
                    relations: ['order', 'orderDetails']
                );
                if ($updatedRefund) {
                    app(FinancePostingService::class)->postRefund($updatedRefund);
                }
            }
