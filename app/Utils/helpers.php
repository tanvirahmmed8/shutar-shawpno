...
//        return (($price-$discount) / 100) * $tax; //after discount decrease
    ...
    public static function module_permission_check($mod_name)
    {
        ...
        if ($mod_name === $purchaseModuleKey && !config('purchase.enabled')) {
            return false;
        }

        $financeModuleKey = config('accounts_finance.module_key', 'accounts_finance');
        if ($mod_name === $financeModuleKey && !config('accounts_finance.enabled')) {
            return false;
        }
        ...
        $permission = $role->module_access;
        $accessList = $permission ? (array) json_decode($permission, true) : [];

        return in_array($mod_name, $accessList, true);
    }
