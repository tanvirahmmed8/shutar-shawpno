...
class OrderManager
{
    ...
    public static function stock_update_on_order_status_change($order, $status)
    {
        ...
        foreach ($order->details as $detail) {
            ...
            $isReturnStatus = in_array($status, ['returned', 'failed', 'canceled']);

            if ($product && $product->product_type === 'physical') {
                if ($isReturnStatus && $detail['is_stock_decreased'] == 1) {
                    $lotInventoryService->releaseForOrderDetail($detail);
                    if ($detail['variant']) {
                        self::adjustVariantInventory($product, $detail['variant'], $detail['qty'], 'increment');
                    }
                } elseif (! $isReturnStatus && $detail['is_stock_decreased'] == 0) {
                    $lotInventoryService->reserveForOrderDetail($detail);
                    if ($detail['variant']) {
                        self::adjustVariantInventory($product, $detail['variant'], $detail['qty'], 'decrement');
                    }
                }
            }
    ...
    public static function generate_order($data)
    {
        ...
        $cartItems = CartManager::getCartListQuery(groupId: $data['cart_group_id'], type: 'checked');
        $lotInventoryService = app(LotInventoryService::class);

        foreach ($cartItems as $cartSingleItem) {
            $productForCheck = Product::select('id', 'product_type')->find($cartSingleItem['product_id']);
            if ($productForCheck && $productForCheck->product_type === 'physical') {
                $lotInventoryService->assertSufficientStock($productForCheck->id, $cartSingleItem['quantity']);
            }
        }
        ...
        DB::transaction(function () use ($data, $or, $order_id, $getCustomerID, $cartItems, $lotInventoryService) {
            ...
            foreach ($cartItems as $cartSingleItem) {
                $productModel = Product::with(['digitalVariation', 'clearanceSale' => function ($query) {
                    return $query->active();
                }])->where(['id' => $cartSingleItem['product_id']])->lockForUpdate()->first();
                ...
                $orderDetail = OrderDetail::query()->create($orderDetails);

                if ($productModel->product_type === 'physical') {
                    if ($cartSingleItem['variant']) {
                        self::adjustVariantInventory($productModel, $cartSingleItem['variant'], $cartSingleItem['quantity'], 'decrement');
                    }
    ...
    public static function order_insert($order_data): int
    {
        ...
        $cartItems = CartManager::getCartListQuery(groupId: $order_data['data']['cart_group_id']);
        $lotInventoryService = app(LotInventoryService::class);
        ...
        DB::transaction(function () use ($or, $order_data, $cartItems, $lotInventoryService) {
            DB::table('orders')->insertGetId($or);
    ...
    private static function adjustVariantInventory(?Product $product, ?string $variantType, float $quantity, string $operation = 'decrement'): void
    {
        if (! $product || ! $variantType || empty($product->variation)) {
            return;
        }

        $variants = json_decode($product->variation, true);
        if (! is_array($variants)) {
            return;
        }

        $quantity = max(0, (float) $quantity);
        $delta = $operation === 'increment' ? $quantity : -$quantity;
        $updated = false;

        foreach ($variants as &$variant) {
            if (($variant['type'] ?? null) === $variantType) {
                $currentQty = (float) ($variant['qty'] ?? 0);
                $newQty = $currentQty + $delta;
                $variant['qty'] = $newQty < 0 ? 0 : round($newQty, 4);
                $updated = true;
                break;
            }
        }
        ...
        if ($updated) {
            Product::whereKey($product->id)->update([
                'variation' => json_encode($variants),
            ]);
        }
    }
