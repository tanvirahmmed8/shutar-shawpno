...
trait InHouseTrait
{
    ...
    public function getInHouseShopObject(): Shop
    {
        ...
        $vacationStatus = (bool) ($inHouseVacation['status'] ?? false);

        $current_date = date('Y-m-d');
        $is_vacation_mode_now = ($vacationStatus && $vacationStartDate && $vacationEndDate && ($current_date >= $vacationStartDate) && ($current_date <= $vacationEndDate)) ? 1 : 0;
